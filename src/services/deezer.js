...
export class DeezerCore {
    ...
    #getIfHasError = (response) => {
        ...
        if (Array.isArray(response.body.error))
            return response.body.error.length > 0 ? response.body.error[0] : null;
    ...
    validatorQueue = new AsyncQueue('validatorQueue', 1, async (now) => {
        if (this.#validatorData.queries.length === 50)
            await sleep(this.#validatorData.expires - Date.now()).then(() =>
                Promise.all(this.#validatorData.queries)
            );
        if (this.#validatorData.expires <= (now = Date.now()))
            this.#validatorData = { expires: now + 5000, queries: [] };
        return new Promise((res) =>
            this.#validatorData.queries.push(new Promise((res_) => res(res_)))
        );
    });
    ...
    #sendRequest = async (ref, opts, retries) => {
        ...
        return this.requestObject
            ...
            .then((response, error) => {
                if (
                    (error = this.#getIfHasError(response)) &&
                    error.code === 4 &&
                    error.message === 'Quota limit exceeded'
                ) {
                    error[this.#retrySymbol] = retries.prior + 1;
                    if (retries.remaining > 1)
                        return this.#sendRequest(ref, opts, {
                            prior: retries.prior + 1,
    ...
    processID(gnFn) {
        return (id, opts) =>
            this.wrappedCall(this.#sendRequest(gnFn(id), opts, this.totalTrials || 5));
    }
        ...
        const wrapPagination = (id, wrpFnx, pagedURL, opts) =>
            pagedURL
                ? () =>
                      wrpFnx(
                          id,
                          (({ index, limit }) => ({ index, limit: limit || opts.limit }))(
                              url.parse(pagedURL, true).query
                          )
                      )
                : null;
        const decoyProcessor = async (id, opts = {}) => {
            const itemObject = await gnFn(id, {
                index: opts.index || 0,
                limit: Math.min(opts.limit, 300) || 300,
            });
...
export default class Deezer {
    ...
    parseURI(uri, storefront) {
        ...
        return {
            ...
            type,
            uri: `deezer:${type}:${id}`,
            url: `https://www.deezer.com/${storefront}/${type}/${id}`,
            storefront,
        };
    }
    ...
    wrapTrackMeta(trackInfo, albumInfo = {}) {
        ...
        const featuring = trackInfo.contributors
            ? trackInfo.contributors
                  .filter((c) => c.role && c.role.toLowerCase().includes('feat'))
                  .map((c) => c.name)
            : [];

        // Extract artist sort names
        const artistSortNames = [trackInfo.artist.name, ...featuring].map(
            (name) => name.split(' ').reverse().join(', ') // Simple sort name transformation
        );
        ...
        return {
            ...
            total_discs:
                albumInfo.tracks?.reduce(
                    (acc, track) => Math.max(acc, track.altData?.DISK_NUMBER || 1),
                    1
                ) || 1,
    ...
    wrapAlbumData(albumObject, altAlbumObject) {
        ...
        let altTracks = Object.fromEntries(
            (altAlbumObject.SONGS?.data || []).map((track) => [track.SNG_ID, track])
        );
        return {
            id: albumObject.id,
            ...
            images: [
                albumObject.cover_small,
                albumObject.cover_medium,
                albumObject.cover_big,
                albumObject.cover_xl,
            ],
            label: albumObject.label,
            release_date: new Date(albumObject.release_date),
            ntracks: albumObject.nb_tracks,
            tracks: albumObject.tracks.data.map((track) => ({
                ...track,
                altData: altTracks[track.id],
            })),
            getImage(width, height) {
                ...
                return this.images
                    ...
                    .replace(
                        /(?<=.+\/)\d+x\d+(?=.+$)/g,
                        `${min(width, 1800)}x${min(height, 1800)}`
                    );
            },
        };
    }
    ...
    createDataProcessor(coreFn) {
        return async (uri) => {
            ...
            if (!this.#store.cache.has(parsed.uri))
                this.#store.cache.set(parsed.uri, await coreFn(parsed.id));
    ...
    artistQueue = new AsyncQueue(
        ...
        4,
        this.createDataProcessor(async (id) =>
            this.wrapArtistData(await this.#store.core.getArtist(id))
        )
    );
    ...
    playlistQueue = new AsyncQueue(
        ...
        4,
        this.createDataProcessor(async (id) =>
            this.wrapPlaylistData(await this.#store.core.getPlaylist(id, { limit: 1 }))
        )
    );
    ...
    async getArtistAlbums(uris) {
        ...
        return this.wrapPagination(
            () =>
                this.#store.core.getArtistAlbums(artist.id, {
                    limit: Math.min(artist.nalbum, Math.max(300, artist.nalbum / 4)),
                }),
    ...
    async getPlaylistTracks(uri) {
        ...
        return this.wrapPagination(
            () =>
                this.#store.core.getPlaylistTracks(playlist.id, {
                    limit: Math.min(playlist.ntracks, Math.max(300, playlist.ntracks / 4)),
                }),
