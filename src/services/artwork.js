 ...
 * Uses iTunes (primary) and Deezer (fallback) to find higher quality covers
 */
export default class ArtworkService {
    ...
    static async searchITunes(artist, album) {
        ...
        if (response.resultCount > 0) {
            ...
            if (result.artworkUrl100) {
                // Magic replacement for high-res (replacing 100x100 with 3000x3000)
    ...
    /**
     * Search Deezer API for album artwork
     * @param {string} artist
     * @param {string} album
     * @returns {Promise<string|null>}
     */
