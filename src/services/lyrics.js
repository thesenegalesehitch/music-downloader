...
export default class LyricsService {
     ...
     * Note: This is a rough estimation. Without audio analysis,
     * we cannot guarantee synchronization. This is a fallback.
    ...
    static generateBasicLRC(plainLyrics) {
        ...
        // This is purely heuristic and will likely not match the song perfectly.
        const lrcLines = [];
        let currentTime = 0;
        const avgCharsPerSecond = 15; // Assumption: ~15 chars sung per second on average
        ...
        for (const line of lines) {
            ...
            // Minimum 2 seconds per line to avoid too fast scrolling
    ...
    static async fetchFromLrcLib(title, artist, album, duration) {
        try {
            ...
            const response = await got(url, {
                ...
                headers: {
                    'User-Agent':
                        'MusicDownloader/1.0.0 (https://github.com/thesenegalesehitch/music-downloader)',
                },
    ...
    static async enrichWithLyrics(track) {
        ...
        const trackArtist =
            typeof artist === 'string'
                ...
                : (artists && artists[0]?.name) ||
                  (Array.isArray(artists) ? artists[0] : 'Unknown');
        ...
        let lyricsData = await this.fetchFromLrcLib(
            trackTitle,
            trackArtist,
            trackAlbum,
            trackDuration
        ...
        if (!lyricsData.plain && !lyricsData.lrc) {
            console.log(
                `[Lyrics] LRCLIB failed, trying lyrics.ovh for: ${trackTitle} - ${trackArtist}`
            );
