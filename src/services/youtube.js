...
function _getSearchArgs(artists, track, album, duration) {
    if (typeof track === 'number') [track, duration] = [undefined, track];
    if (typeof album === 'number') [album, duration] = [undefined, album];
...
export class YouTubeMusic {
    ...
    #search = async function search(queryObject, params) {
        ...
        let { INNERTUBE_API_KEY, INNERTUBE_CLIENT_NAME, INNERTUBE_CLIENT_VERSION } =
            await this.#deriveConfig();
                                  ...
                                  : `other${layerName ? `(${layerName})` : ''}`,
                    {
                        contents: (layer.contents || []).map((content) => {
                            ...
                            let type =
                                layerName === 'Songs'
                                    ? 'song'
                                    : getItemText(content, 1).toLowerCase();
    ...
    async search(artists, track, album, duration) {
        ...
        function calculateAccuracyFor(item, weight) {
            ...
            accuracy +=
                weight - (duration ? Math.abs(duration - item.duration_ms) / duration : 0.5) * 100;
            // if item is a song, bump remaining by 50%, if video, bump up by 25%, anything else - by 5%
            accuracy += ((cur) =>
                ((item.type === 'song' ? 50 : item.type === 'video' ? 25 : 5) / 100) * cur)(
                100 - accuracy
            );
            ...
            validSections.reduce((final, item) => {
                ...
                if (item.weight > 65 && item && 'videoId' in item && !(item.videoId in final)) {
                    let cleanItem = {
                        ...
                        duration_ms:
                            item.duration.split(':').reduce((acc, time) => 60 * acc + +time) * 1000,
                        videoId: item.videoId,
                        getFeeds: genAsyncGetFeedsFn(item.videoId),
                    };
                    if ((cleanItem.accuracy = calculateAccuracyFor(cleanItem, item.weight)) > 80)
...
export class YouTube {
    ...
    async search(artists, track, album, duration) {
        ...
        function calculateAccuracyFor(item) {
            ...
            accuracy +=
                100 -
                (duration ? Math.abs(duration - item.duration.seconds * 1000) / duration : 0.5) *
                    100;
            ...
            accuracy += ((cur) =>
                textUtils.getWeight(strippedArtists, textUtils.stripText([item.author.name])) >= 80
                    ? (60 / 100) * cur
                    : 0)(100 - accuracy);
            return accuracy;
        }
