...
export default class AppleMusic {
    ...
    constructor(config) {
        ...
        for (let instance of [
            this.#store.core.albums,
            this.#store.core.artists,
            this.#store.core.playlists,
        ])
    ...
    loadConfig(config) {
        if (config.developerToken) {
            ...
            this.#store.axiosInstance.defaults.headers['Authorization'] =
                `Bearer ${config.developerToken}`;
        }
    }
    ...
    parseURI(uri, storefront) {
        ...
        const type = isURI
            ? match[4]
            : collection_type == 'album' && parsedURL.query.i
              ? 'track'
              : collection_type;
        const scope =
            collection_type == 'track' || (collection_type == 'album' && parsedURL.query.i)
                ? 'song'
                : collection_type;
        storefront =
            match[1] ||
            storefront ||
            (#store in this ? this.#store.defaultStorefront : null) ||
            'us';
        return {
            id,
            type,
    ...
    wrapTrackMeta(trackInfo, albumInfo = {}) {
        ...
        const artistSortNames = trackInfo.relationships?.artists?.data?.map(
            (artist) =>
                artist?.attributes?.sortName ||
        ...
        return {
            ...
            total_discs:
                albumInfo.tracks?.reduce(
                    (acc, track) => Math.max(acc, track.attributes?.discNumber || 1),
                    1
                ) || 1,
    ...
    async processData(uris, max, store, coreFn) {
        ...
        if (packs.length)
            (
                ...
                .forEach((item) =>
                    item
                        ? (this.#store.cache.set(uris[item.id].uri, item),
    ...
    async getTrack(uris, store) {
        return this.processData(uris, 300, store, async (items, storefront) => {
            ...
            await this.getAlbum(
                tracks.flatMap((item) =>
                    item.relationships.albums.data.map((item) => `apple_music:album:${item.id}`)
                ),
                storefront
            );
            return Promise.mapSeries(tracks, async (track) => {
                track.artists = await this.depaginate(
                    ...
                    async (nextUrl) =>
                        await this.#store.core.songs.get(
                            `${track.id}${nextUrl.split(track.href)[1]}`,
                            { storefront }
                        )
                );
            ...
            Promise.mapSeries(
                ...
                async (artist) => {
                    artist.albums = await this.depaginate(artist.relationships.albums, (nextUrl) =>
                        this.#store.core.artists.get(
                            `${artist.id}${nextUrl.split(artist.href)[1]}`,
                            { storefront }
                        )
                    );
    ...
    async getPlaylist(uris, store) {
        return this.processData(uris, 25, store, async (items, storefront) =>
            Promise.mapSeries(
                ...
                async (playlist) => {
                    playlist.tracks = await this.depaginate(
                        playlist.relationships.tracks,
                        (nextUrl) =>
                            this.#store.core.playlists.get(
                                `${playlist.id}${nextUrl.split(playlist.href)[1]}`,
                                { storefront }
                            )
                    );
