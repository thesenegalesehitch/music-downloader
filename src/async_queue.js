...
export default class AsyncQueue {
        ...
        this.#store.queue = async.queue(({ data, args }, cb) => {
            (async () =>
                ...
                .catch((err) => {
                    ...
                    err[AsyncQueue.debugStack].push({
                        queueName: this.#store.name,
                        sourceTask: data,
                        sourceArgs: args,
                    });
                    cb(err);
    ...
    #_register = (objects, meta, handler) => {
        const promises = (Array.isArray(objects) ? objects : [[objects, meta]]).map(
            (objectBlocks) => {
                const [data, args] = Array.isArray(objectBlocks)
                    ? objectBlocks
                    : [objectBlocks, meta];
                return insulate(
    ...
    async cleanup() {
        if (this.#store.worker && this.#store.worker[IsProvisionedProxy])
            await this.#store.worker(ResourceDropSignal);
    }
