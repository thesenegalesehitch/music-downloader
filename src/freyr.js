...
export default class MusicDownloaderCore {
    ...
    static getBitrates() {
        return Array.from(
            new Set(
                this.ENGINES.reduce(
                    (stack, engine) => stack.concat(engine[symbols.meta].BITRATES || []),
                    []
                )
            )
    ...
    static identifyService(content) {
        return this.ENGINES.find((engine) =>
            engine[symbols.meta].PROPS.isQueryable
                ? content.match(engine[symbols.meta].VALID_URL)
                : undefined
        );
    }
    ...
    static sortSources(includeOrder, exclude) {
        includeOrder = includeOrder
            ? Array.isArray(includeOrder)
                ? includeOrder
                : [includeOrder]
            : [];
        return _sortBy(this.collateSources(), (source) =>
            ((index) => (index < 0 ? Infinity : index))(
                includeOrder.indexOf(source[symbols.meta].ID)
            )
    ...
    constructor(ServiceConfig, AuthServer, serverOpts) {
        ...
        this.ENGINES = MusicDownloaderCore.ENGINES.map(
            (Engine) => new Engine(ServiceConfig[Engine[symbols.meta].ID], AuthServer, serverOpts)
        );
    }
