...
const hookupListeners = () =>
    !hookedUpListeners && (hookedUpListeners = true)
        ? process
              .addListener('beforeExit', garbageCollect.bind(null, void 0))
              .addListener('exit', () => {
                  let err = new Error(
                      '[file_mgr] Somehow, there are still files that need to be removed'
                  );
...
export default function genFile(opts) {
    let inner = async (mode) => {
        ...
        opts = Object.assign(
            { path: null, filename: null, dirname: null, tmpdir: true, keep: false },
            opts
        );
        if (opts.path && (opts.filename || opts.dirname))
            throw new Error('Cannot specify path and either filename or dirname');
