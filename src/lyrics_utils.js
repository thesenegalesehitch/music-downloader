...
export function convertTTMLtoLRC(ttml, options = {}) {
    ...
    for (const p of lines) {
        ...
        const totalMs =
            timestamp.getSeconds() * 1000 + timestamp.getMinutes() * 60 * 1000 + adjustedMs;
        const mins = Math.floor(totalMs / 60000);
        const secs = Math.floor((totalMs % 60000) / 1000);
        const hundredths = Math.floor((totalMs % 1000) / 10);

        lrcLines.push(
            `[${String(mins).padStart(2, '0')}:${String(secs).padStart(2, '0')}.${String(hundredths).padStart(2, '0')}]${text}`
        );
    }
...
export function createLRCFile({ title, artist, album, lyrics }) {
    const lines = [`[ti:${title}]`, `[ar:${artist}]`, `[al:${album}]`, '', lyrics];
